...
export function PwaInstallModal({ platform, onClose }: PwaInstallModalProps) {
    ...
    switch (platform.toLowerCase()) {
      case 'android':
        return {
          ...
          icon: <Smartphone className="text-success" size={48} />,
          steps: [
      ...
      case 'ios':
        return {
          ...
          icon: (
            <svg viewBox="0 0 24 24" className="w-12 h-12 text-muted">
      ...
      case 'windows':
        return {
          ...
          icon: (
            <svg viewBox="0 0 24 24" className="w-12 h-12 text-accent">
      ...
      default:
        return {
          ...
          icon: <Download className="text-accent" size={48} />,
          steps: [
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn">
      <div className="bg-surface border border-line rounded-2xl max-w-lg w-full p-6 shadow-2xl animate-scaleIn">
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            {instructions.icon}
            <div>
              <h2 className="text-2xl font-bold text-ink">{instructions.title}</h2>
              <p className="text-muted text-sm">Progressive Web App</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
            ...
            <ol className="space-y-2.5">
              {instructions.steps.map((step, index) => (
                <li key={index} className="flex gap-3 text-muted text-sm">
                  <span className="flex-shrink-0 w-6 h-6 bg-accent/10 border border-accent/20 rounded-full flex items-center justify-center text-accent text-xs font-semibold">
                    {index + 1}
                  </span>
          ...
          <div className="bg-surface2 border border-line rounded-lg p-4">
            <p className="text-muted text-sm">
              <strong className="font-semibold text-ink">Note:</strong> {instructions.note}
            </p>
          </div>
